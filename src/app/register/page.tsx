...
export default function RegisterPage(){
 ...
 return <main className={styles.page}><section className={styles.visual}><div className={styles.overlay}/><div className={styles.visualCopy}><span>ÚNETE A LA MESA</span><h1>Come mejor. Descubre más.</h1><ul><li><Check/>Guarda tus favoritos</li><li><Check/>Recibe recomendaciones</li><li><Check/>Planea tu próxima salida</li></ul></div></section><section className={styles.panel}><div className={styles.formWrap}><span className={styles.eyebrow}>CREA TU PERFIL</span><h2>Empieza a explorar</h2><p>Tu cuenta convierte Come en una guía personal.</p>{error&&<div className={styles.error}>{error}</div>}<AccesoSocial modo="registrar" onListo={finish} onError={setError} deshabilitado={busy}/><div className={styles.divider}><span>o usa tu correo</span></div><form onSubmit={register}><label>Nombre<div><UserRound size={18}/><input value={name} onChange={e=>setName(e.target.value)} required placeholder="¿Cómo te llamas?"/></div></label><label>Correo electrónico<input type="email" value={email} onChange={e=>setEmail(e.target.value)} required placeholder="tu@correo.com"/></label><label>Contraseña<input type="password" minLength={6} value={password} onChange={e=>setPassword(e.target.value)} required placeholder="Mínimo 6 caracteres"/></label><button className={styles.submit} disabled={busy}>{busy?"Creando cuenta…":"Crear mi cuenta"}<ArrowRight size={18}/></button></form><p className={styles.switch}>¿Ya tienes cuenta? <Link href="/login">Inicia sesión</Link></p></div></section></main>
}
